...
if __name__ == "__main__":
    ...
    search_response = asyncio.run(
        web_search.execute(
            query="I want to design a sofa. What are the most popular styles right now?", fetch_content=True, num_results=1
        )
    )
    # 修改这里：直接打印 output 属性，或者使用 model_dump()
    print(search_response.output)
    
    # 如果你想看完整的 JSON 结构，可以用这个：
